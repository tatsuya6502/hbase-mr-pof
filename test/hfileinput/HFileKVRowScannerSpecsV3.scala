...
import hfileinput.v2.{KeyValueRowMergeScanner, KeyValueHashSet}
...
import org.apache.hadoop.hbase.io.hfile.{HFileRowScanner, BlockCache, LruBlockCache}
import org.apache.hadoop.hbase.util.Bytes

import scala.collection.JavaConversions._

object HFileKVRowScannerSpecsV3 {
  ...
  def runTest(testDataDesc: TestDataDescriptor, useCache: Boolean): TestResult = {
    ...
    storeInfoList.map(_.setRootDir(TestDataDescriptor.v3BaseDir))
...
//      println(storeInfo.getStoreFilePaths.mkString("\n"))
      ...
      val scanners = storeInfo.createKeyValueRowScannersV3(cache)
      for (s <- scanners) {
